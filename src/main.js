...
async function init() {
  ...
  try {
    ...
    const STORAGE_KEY = "clinical_copilot_v9_transcript_tail";
    const MAX_BUBBLES = 8;
    ...
    const TOKEN = String(import.meta.env.VITE_SECRET_TOKEN || "").trim();
    const API_BASE = "https://talky.drewssillytavern.win";
    const WS_BASE = "wss://talky.drewssillytavern.win";

    if (!TOKEN) {
      throw new Error("Missing VITE_SECRET_TOKEN");
    }
    ...
    function pushBubble(type, short, expandedText = "") {
      const clean = normalizeDisplayText(short);
      if (!clean) return;
      if (bubbles.some((b) => b.short === clean)) return;

      bubbles.unshift({
        type: type || "insight",
        short: clean,
        expanded: normalizeDisplayText(expandedText || clean),
      });
    ...
    function buildScreen() {
      ...
      if (expanded === "exit") {
        ...
        lines.push("");
        lines.push(centerStr("2x tap to cancel", W));
        return lines.join("\n");
      }

      if (expanded === "title") {
        lines.push(centerStr(conversationTitle, W - 1) + flash);
        lines.push("");
        const summaryLines = wrapText(oneLiner || "Listening...", W);
        for (const l of summaryLines) lines.push(l);
        lines.push("");
        lines.push(centerStr("tap or 2x to close", W));
        return lines.join("\n");
      }
