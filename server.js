...
// Existing Talky audio websocket
const wss = new WebSocketServer({ server, path: "/stream-audio" });

// New lightweight OcuClaw websocket relay
const ocuWss = new WebSocketServer({ server, path: "/ws" });
...
const OCUCLAW_MODEL = process.env.OCUCLAW_MODEL || "gpt-4o-mini";
...
app.get("/", (_req, res) => {
  res.json({
    ...
    status: "running",
    websocket_paths: ["/stream-audio", "/ws"],
  });
});
...
function extractOcuClawText(msg) {
  ...
  if (Array.isArray(msg.messages)) {
    const joined = msg.messages
      .map((m) => normalizeWS(m?.content || m?.text || ""))
      .filter(Boolean)
      .join("\n");
...
async function createOcuClawReply({
  text,
  history = [],
  systemPrompt = "",
  model = OCUCLAW_MODEL,
  ...
  const cleanSystemPrompt = normalizeWS(systemPrompt);
  if (cleanSystemPrompt) {
    messages.push({ role: "system", content: cleanSystemPrompt });
  } else {
    messages.push({
      role: "system",
      content: "You are a helpful, concise assistant for AR smart glasses.",
    });
  }
    ...
    for (const item of history.slice(-12)) {
      const role =
        item?.role === "assistant" || item?.role === "system"
          ? item.role
          : "user";
      const content = normalizeWS(item?.content || item?.text || "");
      if (content) {
  ...
  return normalizeWS(response.choices?.[0]?.message?.content || "");
}
...
// ───────────────────────────────────────────────────────────────────────────────
// OcuClaw WebSocket relay
// ───────────────────────────────────────────────────────────────────────────────
ocuWss.on("connection", async (ws, req) => {
  const queryToken = getTokenFromReq(req);
  const state = {
    authed: !SECRET_TOKEN || queryToken === SECRET_TOKEN,
    history: [],
    model: OCUCLAW_MODEL,
    systemPrompt: "",
  };

  if (SECRET_TOKEN && !state.authed) {
    console.log("OcuClaw connection awaiting auth message");
    sendWsJson(ws, {
      type: "auth_required",
      ok: false,
    ...
    console.log("OcuClaw client connected");
    sendWsJson(ws, {
      type: "connected",
      ok: true,
      model: state.model,
  ...
  ws.on("message", async (raw, isBinary) => {
    if (isBinary) {
      sendWsJson(ws, {
        type: "error",
        ok: false,
        message: "Binary audio is not supported on /ws",
      });
      return;
    }
    ...
    if (!state.authed) {
      const incomingToken =
        normalizeWS(msg.token || msg.apiKey || msg.api_key || "") ||
        msg?.auth?.token ||
        "";
    ...
    if (msg.type === "config" || msg.type === "settings") {
      if (msg.model) state.model = String(msg.model).trim();
      if (msg.systemPrompt || msg.system_prompt) {
        state.systemPrompt = normalizeWS(
          msg.systemPrompt || msg.system_prompt || ""
        );
      }
    ...
    if (msg.type === "reset" || msg.type === "clear") {
      state.history = [];
      sendWsJson(ws, { type: "reset_ack", ok: true });
      return;
    }

    const text = extractOcuClawText(msg);
    ...
    try {
      ...
      const reply = await createOcuClawReply({
        text,
        history: Array.isArray(msg.history) ? msg.history : state.history,
        systemPrompt:
          msg.systemPrompt || msg.system_prompt || state.systemPrompt,
      ...
      console.error("OcuClaw relay error:", err);
      sendWsJson(ws, {
        type: "error",
        ok: false,
  ...
  ws.on("close", () => {
    console.log("OcuClaw client disconnected");
  });

  ws.on("error", (err) => {
    console.error("OcuClaw WS error:", err.message);
  });
});

// ───────────────────────────────────────────────────────────────────────────────
// Talky WebSocket handler
// ───────────────────────────────────────────────────────────────────────────────
...
server.listen(port, "0.0.0.0", () => {
  ...
  console.log(`OcuClaw relay path: /ws`);
  console.log(`OcuClaw model: ${OCUCLAW_MODEL}`);
});
