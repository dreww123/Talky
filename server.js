...
const ANALYSIS_INTERVAL_MS = Number(process.env.ANALYSIS_INTERVAL_MS) || 15000;
const MIN_NEW_CHARS_FOR_ANALYSIS =
  Number(process.env.MIN_NEW_CHARS_FOR_ANALYSIS) || 80;

const MIN_CONFIDENCE_TO_SURFACE = Number(
  process.env.MIN_CONFIDENCE_TO_SURFACE ?? 0.82
);

const BUBBLE_SIMILARITY_THRESHOLD = Number(
  process.env.BUBBLE_SIMILARITY_THRESHOLD ?? 0.72
);
...
function normalizeAnalysis(parsed) {
  ...
  const content = clamp(normalizeWS(parsed.content || ""), 1200);
  ...
  return {
    ...
    answer: isQuestion ? oneLiner : "",
    answer_expanded: isQuestion ? content : "",

    insight: !isQuestion && shouldSurface ? oneLiner : "",
...
{
  "title": "≤5 words",
  "summary": "short preview, ≤14 words",
  "category": "question | factcheck | insight | reference | none",
  "content": "2-4 dense expert sentences, 60-140 words, include the non-obvious why, the competing concern, and what changes management",
  "confidence": 0.0-1.0
}
...
1. If a direct clinical question is asked:
   ...
   - summary = very short answer preview
...
- Specific
- Non-obvious
- Short preview in summary, fuller reasoning in content
...
async function analyzeTranscript({
  ...
  const prompt = buildPrompt({
    recentTranscript: clamp(recentTranscript, 7000),
    transcriptWindow: clamp(transcriptWindow, 18000),
    priorSummary: clamp(priorSummary || "", 400),
